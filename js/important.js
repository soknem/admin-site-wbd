...
export function handleAddButtonClick() {
  ...
  const attachInputEventListener = () => {
    const textInput = rightSide.querySelector(
      ".main-important .add-container .input-container .text-input"
    );
    ...
    if (textInput && textInput.value.trim() !== "") {
      try {
        ...
        if (newTodo && newTodo.uuid) {
          const ul = rightSide.querySelector(".main-important .data .ul");
          addTodoToList(ul, newTodo, "important");
          handleSidebarCountLoading();
...
export async function handleLoadContentToImportant() {
  const rightSide = document.getElementById("right-side");
  if (!rightSide) {
...
async function loadTodos(ul) {
  try {
    showLoading(true);
    const todos = await getTodos("important");
    ...
    todos.forEach((todo) => {
      ...
      addTodoToList(ul, todo, "important");
    });
  } catch (error) {
...
export async function handleIsDoneCountLoading() {
  ...
  const doneCount = rightSide.querySelector(
    ".main-important .data .drop-down-header .done-count"
  );

  if (doneCount) {
    const myDayCountIsDone = await getTodoCountIsDone("important");
